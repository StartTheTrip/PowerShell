...
Function Get-RemoveString($Location="",$stringToRemove="DONOTEDITTHISPROTECTION") {
	...
	$preString = Read-Host -Prompt "Enter text you want removed from file name, e.g. 01-01-1999"
	if ($preString -eq '') {
    ...
    if($foldername.ShowDialog() -eq "OK")
    {
		...
		Get-ChildItem -Path $folder -File | Rename-Item -NewName {$_.Name -replace $stringToRemove, "" }
